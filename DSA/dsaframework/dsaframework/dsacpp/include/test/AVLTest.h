...
TEST_CASE( "AVL<int, int*>: removing items" ) {
    ...
    SUBCASE("Removing => no re-balance"){
        ...
        bool found;
        tree.remove(20, &found);
        REQUIRE(tree.size() == 6);
        
        tree.remove(40, &found);
        REQUIRE(tree.size() == 5);
    ...
    SUBCASE("Removing: EH-of-LH => single rotation: rotateRight"){
        ...
        bool found;
        tree.remove(60, &found);
        tree.remove(80, &found);
        tree.remove(70, &found);
        REQUIRE(tree.size() == 4);
    ...
    SUBCASE("Removing: until emtpy"){
        ...
        bool found;
        for(int idx=0; idx < 11; idx++) tree.remove(keys[idx], &found);
        REQUIRE(tree.size() == 0);
