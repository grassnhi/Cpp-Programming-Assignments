...
{ 
    ...
    {
        history->recentTable = history->insert(history->recentTable,i,"nhi",2*i);
    }
    ...
    if(merged == nullptr || (merged != nullptr && ID != merged->count)){
        ...
        if(curr->name != ""){
            ...
            if(queue->recentTable != nullptr){ // add queue vào (add head vào)
                ...
                history->recentTable = history->insert(history->recentTable, ID, curr->name, curr->age); //update his
            }
        }  
