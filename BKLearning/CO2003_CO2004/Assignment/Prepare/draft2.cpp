...
void outOfTable(int result){
    if(result % 3 == 0){
        int tableID = FIFO->front();
        FIFO->pop();
        FIFO->push(tableID);
        orders++;
        if(hashMap->findID(tableID)){
...
void REG(string name){
    ...
    // check for table or food
    int check = LFCO->existName(result);
    if(check = -1){ 
        // region and table
        placeTable(result);
    }else{
        LFCO->increaseNUM(check);
    }
}
